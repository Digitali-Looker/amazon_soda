      ...
      WHERE
              ...
              and
                  {% if paneldata.date_date._is_filtered %}
                   (date_from_parts(fw.panelyear, fw.panelquarter*3-2,1) >= trunc({% date_start paneldata.date_date %},'quarter')
                  and
                  date_from_parts(fw.panelyear, fw.panelquarter*3-2,1) <= trunc(dateadd(day,-1,{% date_end paneldata.date_date %}), 'quarter'))
