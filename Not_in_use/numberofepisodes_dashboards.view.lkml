view: numberofepisodes_dashboards {
  ####################
##DS 05/03/20
##replica from numberofepisodes for client explore
####################
    ...
    derived_table: {
      sql: select distinct e.Netflixid
              ...
              --
            where
              {% condition paneldata.date_raw  %} pdq.date  {% endcondition  %}
              and
              {% condition paneldata.countrynameformaps %} CASE WHEN pdq.country = 'UK' THEN 'United Kingdom' ELSE  pdq.country END {% endcondition %}
              --
              group by
...
######################################################################################
    ...
    dimension: noofepisodes {
      ...
      sql: IFNULL({%if episodes.episodenumber_IFNULL._in_query or paneldata.content_granularity._parameter_value == 'Episode'%}
         ${TABLE}.EpisodeLevel
