view: releasedate_dashboards {
  ####################
## DS 05/03/20
## Replica of the view for client facing explore
## Incorporates the use of content granularity for dashboards
####################
...
#   sql_start: ${releasedate.datefirstviewed};;
      sql_start: ${releasedate_dashboards.datefirstviewed} ;;
...
#             case when e.SEASONNUMBER is null and count(tm.Netflixid) OVER(PARTITION BY tm.NETFLIXID, a.COUNTRY )>1 then 1 else 0 end as checktwo -- exclude those with partially missing episodeids
#       FROM core.TitleMaster tm
